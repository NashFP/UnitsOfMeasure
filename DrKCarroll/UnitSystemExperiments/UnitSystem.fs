...
type Quantity (b:float, dimensions:Dimensions) = 
    ...
    member this.IsNotDimensionless () = this.Dimensions |> Array.exists ((<>) 0y)
...
//               ) + "]"
    ...
    static member Pow (mantissa:Quantity, exponent:int) =
        Quantity(Math.Pow(mantissa.BaseValue, float exponent), mantissa.Dimensions |> Array.map ((*) (sbyte exponent)))
